...
impl SolanaTokenClient {
    pub fn new(rpc_url: &str, commitment: &str, keypair_path: &str) -> anyhow::Result<Self> {
        ...
        let authority = read_keypair_file(keypair_path)
            .with_context(|| format!("read keypair at {keypair_path}"))?;
        Ok(Self {
            rpc,
    ...
    pub fn create_mint(&self, decimals: u8) -> anyhow::Result<(Pubkey, Signature)> {
        ...
        let recent = self.rpc.get_latest_blockhash().context("get blockhash")?;
    ...
    pub fn burn(&self, mint: Pubkey, owner: Pubkey, amount: u64) -> anyhow::Result<(Pubkey, Signature)> {
